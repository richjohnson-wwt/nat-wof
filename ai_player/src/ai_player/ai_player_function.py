...
# In-process guard to ensure only one action per NAT run, even if Redis guard
# is unavailable or current_game_id is missing. This is reset between runs.
_inproc_turn_completed = False

def _is_inproc_turn_completed() -> bool:
    return _inproc_turn_completed

def _set_inproc_turn_completed(value: bool = True) -> None:
    global _inproc_turn_completed
    _inproc_turn_completed = bool(value)
...
async def spin_wheel_and_guess_consonant(
    ...
    async def _spin_wheel_and_guess_consonant(input_text: str) -> str:
        ...
        # Guard: prevent multiple actions in one run
        if _is_inproc_turn_completed() or is_turn_completed():
            output = {
                "action": "buy_vowel",
                "success": False,
                "details": "Turn already completed; ignoring extra request.",
                "updates": {"player": player_name},
                "final_answer": "Final Answer: Turn already completed; ignoring extra request.",
            }
        ...
        if wedge is None:
            ...
            _set_inproc_turn_completed(True)
            return json.dumps(output)
...
async def buy_vowel_if_enough_money(
    ...
    # Implement your function logic here
    async def _buy_vowel_if_enough_money(input_text: str) -> str:
...
async def solve_puzzle_if_knows_answer(
    ...
    async def _solve_puzzle_if_knows_answer(input_text: str) -> str:
        ...
        from ai_player.state_manager import get_field, set_current_game_status_finished, reveal_all, is_turn_completed, set_turn_completed
        ...
        if success and proposed_solution:
            fa = f"Final Answer: Solved '{proposed_solution}'."
        elif proposed_solution:
            fa = f"Final Answer: Proposed '{proposed_solution}' but it was incorrect."
        else:
            fa = "Final Answer: No proposed solution provided."
        output = {
            "action": "solve",
            "success": success,
            "details": details,
